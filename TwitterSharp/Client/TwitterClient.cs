...
namespace TwitterSharp.Client
{
    ...
    public class TwitterClient : IDisposable
    {
        ...
        public async Task NextTweetSampleStreamAsync(Func<Tweet, Task> onNextTweet, TweetSearchOptions options = null)
        {
            options ??= new();
            ...
            _tweetStreamCancellationTokenSource = new();
            var res = await _httpClient.GetAsync(_baseUrl + "tweets/sample/stream?" + options.Build(true), HttpCompletionOption.ResponseHeadersRead, _tweetStreamCancellationTokenSource.Token);
            BuildRateLimit(res.Headers, Endpoint.SampledStream);
            _reader = new(await res.Content.ReadAsStreamAsync(_tweetStreamCancellationTokenSource.Token));
            ...
            try
            {
                while (!_reader.EndOfStream && !_tweetStreamCancellationTokenSource.IsCancellationRequested)
                {
                    ...
                    await onNextTweet(ParseData<Tweet>(str).Data);
                }
            }
            catch (IOException e)
            {
        ...
        public async IAsyncEnumerable<Tweet> NextTweetSampleStreamAsync(TweetSearchOptions options = null)
        {
            options ??= new();
            ...
            while (!_reader.EndOfStream && !_tweetStreamCancellationTokenSource.IsCancellationRequested)
            {
                ...
                yield return ParseData<Tweet>(str).Data;
            }
